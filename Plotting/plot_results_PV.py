...
# ../results/Satellite/CP_TV_V/Old/2012-10-13
...
for lambd in lambds1:
    try:
        # file_name = f'../results/Satellite/{method}/Old/{date}/{date}_sigma={sigma}_lambda={lambd}_it={it}_rho=Nan/{date}_sigma={sigma}_lambda={lambd}_it={it}_rho=Nan.mat'
        ...
        SSH_result_sigma1['name'] = method_names[1]
...
for i in range(1,num_subplots):
    ...
    axs[j, i].set_title('Potential Vorticity Variation, \nλ={lamd}'.format(
    ...
    axs[j, i].axis('off')
    axs[1,i].set_title('Performance'.format(name=SSH_results[i-1]['name']), fontsize=25)
    axs[1,i].grid('on')

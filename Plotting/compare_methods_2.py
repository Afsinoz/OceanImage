...
'Files path needs to be checked'
...
for lambd in lambds1:
    try:
        ...
        SSH_result_sigma1=loadmat(f'../Conda_Vu_TV_Vorticity/results/Results/{file_name}',simplify_cells=True)
...
num_subplots = np.max(np.array((len(SSH_CV_TV_V1), len(SSH_CV_TV_V2))))+1

fig, axs = plt.subplots(3, num_subplots, figsize=(5*num_subplots, 15))


SSH_ref = loadmat('../datasets/data_obs/2012-10-13_SSH.mat',simplify_cells=True)
SSH_obs = SSH_ref['SSH_obs']

SSH_ref = SSH_ref['SSH_ref']
